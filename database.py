...
def get_courses_by_professor(professor_id):
    return db.session.query(Course, BaseCourse).join(
        BaseCourse, Course.code == BaseCourse.code
    ).filter(Course.professor_id == professor_id).all()
...
def get_all_courses_with_professors():
    return db.session.query(Course, BaseCourse, Professor).join(
        BaseCourse, Course.code == BaseCourse.code
    ).join(
        Professor, Course.professor_id == Professor.personnel_code
    ).all()
...
def get_courses_with_accepted_tas():
    return db.session.query(Course, BaseCourse, Professor).join(
        ...
        Request, (Request.course_code == Course.code) & 
                 (Request.professor_id == Course.professor_id) & 
...
def get_requests_with_students_by_course(course_code, professor_id, term, status=None):
    query = db.session.query(Request, Student).join(
        Student, Request.student_id == Student.student_number
    ).filter(
...
def get_reviews_with_reviewers(ta_id):
    return db.session.query(Review, Student).join(
        Student, Review.reviewer_id == Student.student_number
    ).filter(Review.ta_id == ta_id).all()



def get_reviews_with_course_and_professor(ta_id):
    return db.session.query(Review, BaseCourse, Professor, Student).join(
        BaseCourse, Review.course_code == BaseCourse.code
    ).join(
        Professor, Review.professor_id == Professor.personnel_code
    ).join(
        Student, Review.reviewer_id == Student.student_number
    ).filter(Review.ta_id == ta_id).all()
...
def get_student_requests_with_details(student_id):
    return db.session.query(Request, BaseCourse, Professor).join(
        BaseCourse, Request.course_code == BaseCourse.code
    ).join(
        Professor, Request.professor_id == Professor.personnel_code
    ).filter(
        Request.student_id == student_id
    ).order_by(Request.term.desc()).all()